...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    def train(model, optimizer, criterion, ctc_a2l_module, ctc_v2l_module, ctc_a2l_optimizer, ctc_v2l_optimizer, ctc_criterion):
        ...
        for i_batch, (batch_X, batch_Y, batch_META) in enumerate(train_loader):
            sample_ind, text, audio, vision, ey = batch_X
            ...
            if hyp_params.use_cuda:
                with torch.cuda.device(0):
                    text, audio, vision, ey, eval_attr = text.cuda(), audio.cuda(), vision.cuda(), ey.cuda(), eval_attr.cuda()
            ...
            if batch_chunk > 1:
                ...
                ey_chunks = ey.chunk(batch_chunk, dim=0)
                eval_attr_chunks = eval_attr.chunk(batch_chunk, dim=0)
                
                for i in range(batch_chunk):
                    text_i, audio_i, vision_i, ey_i = text_chunks[i], audio_chunks[i], vision_chunks[i], ey_chunks[i]
                ...
                preds, hiddens = net(text, audio, vision, ey)
                if hyp_params.dataset == 'iemocap':
